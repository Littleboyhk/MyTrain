...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const trainNumber = String(body.train_number ?? body.trainNumber ?? "12496").trim();
    ...
    const rkDate = toRailkitDate(date);
    const data = await rk.getAvailability(trainNumber, from, to, rkDate, classCode, quota);
